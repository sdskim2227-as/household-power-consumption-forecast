"""
XGBoost로 "다음 날 총 사용량(next_day_energy_kWh)"을 예측한다.
10/11과 같은 입력 변수·같은 분할을 써서 세 모델을 나란히 비교할 수 있게 한다.
원본(train.csv/test.csv)은 읽기만 하고 고치지 않는다.

`seoul-rainfall-prediction`에서는 튜닝 없는 기본 XGBoost가 과적합(train R²=0.996, test R²=-0.02)을
일으킨 전례가 있고, 이 프로젝트는 표본이 1,128행(train)으로 더 적어 과적합 위험이 더
크다 -- train/test R²를 나란히 찍어 과적합 여부를 바로 확인한다.
"""
...
def main() -> None:
    ...
    for path, label in [
        ...
        (OUTPUT_DIR / "rf_regression_metrics.csv", "랜덤포레스트"),
    ]:
        if path.exists():
            print(f"\n=== {label} 대비 ===")
    ...
    result = pd.DataFrame(
        [{"모델": "XGBoost", "RMSE": round(rmse, 3), "MAE": round(mae, 3), "R2": round(r2, 4)}]
    )
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    result.to_csv(OUTPUT_DIR / "xgb_regression_metrics.csv", index=False, encoding="utf-8-sig")
    importances.to_csv(OUTPUT_DIR / "xgb_regression_importances.csv", index=False, encoding="utf-8-sig")
    print(f"\n저장: {(OUTPUT_DIR / 'xgb_regression_metrics.csv').relative_to(ROOT)}")
    print(f"저장: {(OUTPUT_DIR / 'xgb_regression_importances.csv').relative_to(ROOT)}")
